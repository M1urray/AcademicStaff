    ...
    public class LecturerController : Controller
    {
        ...
        public ActionResult CourseAllocationUnits()
        {
            ...
            else
            {
                ...
                string pageLec = "UnitAssociateLecturers?$filter=Lecturer_No eq '" + StaffNo + "' and Semester eq '" + Sem + "'&$format=json";
                ...
                using (var streamReader = new StreamReader(httpResponseLec.GetResponseStream()))
                {
                    ...
                    foreach (JObject config in details["value"])
                    {
                        string pageT = "Timetable?$filter=Semester eq '" + Sem + "' and Unit eq '" + (string)config["Unit_Code"] + "' and Campus_Code eq '" + (string)config["Campus_Code"] + "' and Unit_Class eq '" + (string)config["Class_Code"] + "'&$format=json";
                ...
                return View(LectUnitAllocation.DistinctBy(x => new { x.Unit, x.Semester, x.CourseClass, x.Campus_Code, x.Day, x.Period }).ToList());
            }
        }
        ...
        public ActionResult SpecialExamAllocationUnits()
        {
            if (Session["Username"] == null)
            {
                return RedirectToAction("Login", "Login");
            ...
            else
            {
                ...
                List<LecturerAssignedUnits> LectUnitAllocation = new List<LecturerAssignedUnits>();
                string Sem = CommonClass.ExamSemester();
                string page = "Special_Supp_Allocations?$filter=Lecturer eq '" + StaffNo + "' and Semester eq '" + Sem + "' and Allocation_Type eq 'Special'&$format=json";
            ...
            if (Session["Sem"] != null && Session["Unit"] != null)
            {
                ...
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    ...
                    foreach (JObject config in details["value"])
                    {
                        CustomerList Cust = new CustomerList();
                        Cust.No = (string)config["Student_No"];
                        Cust.Name = CommonClass.GetStudentName((string)config["Student_No"]);
                        Cust.ApplicationNo = (string)config["Application_No"];
                        Cust.Marks = (string)config["Marks_Score"];
                        Cust.Semester = (string)config["Semester_Unit_Done"];
                        studentlist.Add(Cust);
                    }
                }
        ...
        public ActionResult SupplimentaryExamAllocationUnits()
        {
            if (Session["Username"] == null)
            {
                return RedirectToAction("Login", "Login");
            ...
            else
            {
                ...
                List<LecturerAssignedUnits> LectUnitAllocation = new List<LecturerAssignedUnits>();
                string Sem = CommonClass.ExamSemester();
                string page = "Special_Supp_Allocations?$filter=Lecturer eq '" + StaffNo + "' and Semester eq '" + Sem + "' and Allocation_Type eq 'Supplimentary'&$format=json";
        ...
        public ActionResult SupplimentaryExamList()
        {
            if (Session["Username"] == null)
            {
                return RedirectToAction("Login", "Login");
        ...
        public PartialViewResult LoadSupplimentaryExamStudentUnitList()
        {
            if (Session["Sem"] != null && Session["Unit"] != null)
            {
                string StaffNo = Session["Username"].ToString();
                ...
                List<CustomerList> studentlist = new List<CustomerList>();
                string page = "StudentUnits?$filter=Semester eq '" + Sem + "' and Unit eq '" + Unit + "' and Register_for eq 'Supplementary'&$format=json";
                ...
                return PartialView("~/Views/Lecturer/LoadSupplimentaryExamStudentList.cshtml", studentlist.OrderBy(x => x.No).DistinctBy(x => x.No).ToList());
            }
            else
            {
                return PartialView();
        ...
        private string GetMark(string StdNo,string Sem,string Unit)
        {
            ...
            try
            {
                string page = "ExamResults?$filter=Student_No eq '" + StdNo + "' and Semester eq '" + Sem + "' and ExamType eq 'EXAM' and Unit eq '" + Unit + "' and Cancelled eq false&$format=json";
        ...
        public JsonResult SaveStudentMarks(List<Array> headers, List<Array> Rows)
        {
            try
            {
                if (Session["Sem"] != null && Session["Unit"] != null && Session["Campus"] != null)
                {
                    ...
                    //                + Sem + "' and Class_Code eq '" + classCode + "' and Entry_Code eq '" + HeaderText[j].Trim() + "'&$format=json";
                ...
                {
                    string Redirect = "/Lecturer/MarkEntryAllocationUnits";
        ...
        public JsonResult SaveSpecialExamMarks(List<Array> Rows)
        {
            try
            {
                ...
                {
                    string Redirect = "/Lecturer/SpecialExamAllocationUnits";
                    return Json(new { message = Redirect, success = true, failed = true, redirect = true }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
        ...
        [HttpPost]
        public JsonResult SaveSupplimentaryExamMarks(List<Array> headers, List<Array> Rows)
        {
            try
            {
                if (Session["Sem"] != null && Session["Unit"] != null)
                {
                    ...
                    if (SuccV == true)
                    {
                        msg = "Supplimentary exam mark for " + count.ToString() + " saved successfully";
                    }
                    return Json(new { message = msg, success = SuccV }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    string Redirect = "/Lecturer/SupplimentaryExamAllocationUnits";
                    return Json(new { message = Redirect, success = true, failed = true, redirect = true }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
        ...
        public JsonResult GetUnitReports(string ReportType)
        {
            try
            {
                ...
                if (Session["Sem"] != null && Session["Unit"] != null && Session["Campus"] != null)
                {
                    ...
                    if (ReportType == "CLATT")
                    {
                        Credentials.ObjNav.PrintClassList("", Unit, "", Sem, ClassCode, Campus, 1, "CLASSLIST-" + _filename + ".pdf");
            ...
            {
                return Json(new { message = "Problem Encountered while reading report. Contach ICT", success = false }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public JsonResult PrintScoreSheet()
        {
            ...
            try
            {
                if (Session["Sem"] != null && Session["Unit"] != null && Session["Campus"] != null)
                {
                    ...
                    Error success = CommonClass.StartMarkSheettReport(Lec, Prog, Sem, Unit, UnitName, Campus, classCode, ImagePath, rptpath, ProgC);
                    if (success.success)
                    {
        ...
        public JsonResult PrintSupplimentaryScoreSheet()
        {
            bool successVal = false;
            string msg = "";
            try
            {
                if (Session["Sem"] != null && Session["Unit"] != null && Session["Campus"] != null)
                {
                    ...
                    Error success = CommonClass.StartSupplimentaryScoreSheet(Lec, Prog, Sem, Unit, UnitName, Campus, classCode, ImagePath, rptpath, ProgC);
                    if (success.success)
                    {
                        string DestinationPath = rptpath + success.Message;
                        System.IO.FileInfo file = new System.IO.FileInfo(DestinationPath);
        ...
        public ActionResult ExamEntrySetupForm()
        {
            try
            {
                ...
                else
                {
                    ...
                    List<ExamSetupEntry> ExamEntrySetuplist = new List<ExamSetupEntry>();
                    string pageS = "ExamEntrySetup?$filter=Lecturer eq '" + Lec + "' and Unit eq '" + Unit + "' and Semester eq '"
        ...
        public PartialViewResult LoadClassAttendanceList()
        {
            if (Session["Sem"] != null && Session["Unit"] != null && Session["Campus"] != null || Session["ClassCode"] != null)
            {
                ...
                return PartialView("~/Views/Lecturer/LoadAtendanceEntries.cshtml", AttendanceList);
            }
            else
            {
                return PartialView();
            ...
            if (Session["Sem"] != null && Session["Unit"] != null && Session["Campus"] != null || Session["ClassCode"] != null)
            {
                ...
                if (DocNo != "")
                {
                    ...
                    using (var streamReaderTmT = new StreamReader(httpResponseTmTSub.GetResponseStream()))
                    {
                        ...
                        foreach (JObject config1 in detailsTmT["value"])
                        {
                            ...
                            string pageWk = "WeeksList?$select=Code,StartDate&$filter=Code eq '" + Code + "' and Day eq '" + (string)config1["DayofWeek"] +
                                "' and StartDate le " + tday + "&$format=json";
                            ...
                            using (var streamReader = new StreamReader(httpResponseWk.GetResponseStream()))
                            {
                                ...
                                foreach (JObject config in details["value"])
                                {
                                    if (ClassAttendanceWeekTaken(Lec, Sem, Unit, Campus, (string)config["Code"]))
                                    {
                                        ...
                                        ddl.Text = (string)config["Code"] + "-" + (string)config1["DayofWeek"] + "-" + ((DateTime)config["StartDate"]).ToString("dd/MM/yyyy");
                                        WeekList.Add(ddl);
                                    }
                                }
                            }
                ...
                else
                {
                    ...
                    using (var streamReaderTmT = new StreamReader(httpResponseTmT.GetResponseStream()))
                    {
                        ...
                        {
                            string tday = DateTime.UtcNow.ToString("o");
                            string pageWk = "WeeksList?$select=Code,StartDate&$filter=Day eq '" + (string)config1["DayofWeek"] +
                                "' and Inactive eq false and StartDate le " + tday + " and Semester eq '" + Sem + "'&$format=json";
