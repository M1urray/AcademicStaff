    ...
    public class DocumentApprovalController : Controller
    {
                    ...
                    else
                    {
                        ...
                        using (var streamReader = new StreamReader(httpResponsestd.GetResponseStream()))
                        {
                            ...
                            foreach (JObject config in details["value"])
                            {
                                ...
                                if ((string)config["Requisition_Type"] == "Student Defer")
                                {
                                    StdCount.DeferCount = StdCount.DeferCount + 1;
                                }
                                ...
                                if ((string)config["Requisition_Type"] == "Online Application")
                                {
                                    StdCount.Online_App_Count = StdCount.Online_App_Count + 1;
                                }
                            }
                            StdCount.Status = rn;
                        }
        ...
        public ActionResult LoadDocumentForApprovalList(string TbID, string Status, string DocType)
        {
            try
            {
                ...
                if (TbID == "70134894" || TbID == "70134720")
                {
        ...
        public PartialViewResult OnlineApplicationDocApprovalDetails(string DocNo, string Sequence)
        {
            try
            {
                #region Requisition Doc
                Online_App_Header RDoc = new Online_App_Header();
                string page = "QyApplicationFormHeader?$filter=Application_No eq '" + DocNo + "'&$format=json";
                HttpWebResponse httpResponse = Credentials.GetOdataData(page);
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    var result = streamReader.ReadToEnd();
                    ...
                    foreach (JObject config in details["value"])
                    {
                        RDoc.Code = (string)config["Application_No"];
                        RDoc.Names = (string)config["Surname"] + " " + (string)config["Other_Names"];
                        RDoc.Application_Date = ((DateTime)config["Application_Date"]).ToString("dd/MM/yyyy");
                        RDoc.Campus = (string)config["Campus"];
                        RDoc.Intake_Code = (string)config["Intake_Code"];
                        RDoc.Academic_Year = (string)config["Academic_Year"];
                        RDoc.Programme = (string)config["First_Degree_Name"];
                        RDoc.Programme_Level = (string)config["Programme_Level"];
                        RDoc.ModeOfStudy = (string)config["Mode_of_Study"];
                        RDoc.Index_No = (string)config["Index_Number"];
                        RDoc.DoB = ((DateTime)config["Date_Of_Birth"]).ToString("dd/MM/yyyy");
                        RDoc.Gender = (string)config["Gender"];
                        RDoc.Nationality = (string)config["Nationality"];
                        RDoc.Stage = (string)config["Admitted_To_Stage"];
                        RDoc.Phone_No = (string)config["Telephone_No_1"];
                        RDoc.Email = (string)config["Email"];
                    }
                }
                #endregion
                #region Qual Lines
                List<Appl_Qual> QualLines = new List<Appl_Qual>();
                string pageLn = "ApplicationFormQualification?$filter=Application_No eq '" + DocNo + "'&$format=json";
                HttpWebResponse httpResponseLn = Credentials.GetOdataData(pageLn);
                using (var streamReader = new StreamReader(httpResponseLn.GetResponseStream()))
                {
                    var result = streamReader.ReadToEnd();
                    ...
                    if (details["value"].Count() > 0)
                    {
                        foreach (JObject config in details["value"])
                        {
                            Appl_Qual Q = new Appl_Qual();
                            Q.Type = (string)config["Qualification"];
                            Q.Where_Obtained = (string)config["WhereObtained"];
                            Q.Award = (string)config["Award"];
                            Q.From = (string)config["From_Year"];
                            Q.To = (string)config["To_Year"];
                            Q.Graduation_Year = (string)config["Graduation_Year"];
                            QualLines.Add(Q);
                        }
                    }
                }
                ...
                List<Appl_Subjects> SubjLines = new List<Appl_Subjects>();
                string pageSub = "ApplicationFormAcademic?$filter=Application_No eq '" + DocNo + "'&$format=json";
                HttpWebResponse httpResponseSub = Credentials.GetOdataData(pageSub);
                using (var streamReader = new StreamReader(httpResponseSub.GetResponseStream()))
                {
                    var result = streamReader.ReadToEnd();
                    ...
                    if (details["value"].Count() > 0)
                    {
                        foreach (JObject config in details["value"])
                        {
                            Appl_Subjects s = new Appl_Subjects();
                            s.Subject = (string)config["Subject"];
                            s.Grade = (string)config["Grade"];
                            SubjLines.Add(s);
                        }
                    }
                }
                #endregion
                #region Get Pic
                var base64 = CommonClass.ApplicantProfilePicture(DocNo);
                #endregion

                Online_App newDoc = new Online_App
                {
                    Pic = base64,
                    DocD = RDoc,
                    Qual_List = QualLines,
                ...
                return PartialView("~/Views/DocumentApproval/Document Approval Views/Online_Application.cshtml", newDoc);
            }
            catch (Exception ex)
            {
                Error erroMsg = new Error();
        ...
        public JsonResult RejectDocument(string TBID, string DocNo, string Comments, string SeqNo, string EntryNo)
        {
            try
            {
                ...
                Credentials.ObjNav.DocumentRejections(Convert.ToInt32(EntryNo), DocNo, userID, Comments,Convert.ToInt32(TBID), Convert.ToInt32(SeqNo));
                msg = "Approval Request Rejected";
