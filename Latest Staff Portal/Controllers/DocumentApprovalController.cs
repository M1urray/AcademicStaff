    ...
    public class DocumentApprovalController : Controller
    {
        ...
        public PartialViewResult PVReqDocApprovalDetails(string DocNo)
        {
            try
            {
                ...
                string page = "Payment_Header?$filter=No eq '" + DocNo + "'&$format=json";
                HttpWebResponse httpResponse = Credentials.GetOdataData(page);
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    var result = streamReader.ReadToEnd();
                ...
                string pageLine = "Payment_Line?$filter=No eq '" + DocNo + "'&format=json";
                HttpWebResponse httpResponseLine = Credentials.GetOdataData(pageLine);
                using (var streamReader = new StreamReader(httpResponseLine.GetResponseStream()))
                {
                    var result = streamReader.ReadToEnd();
                    ...
                    foreach (JObject config in details["value"])
                    {
                        ...
                        PLine.AccountType = (string)config["Account_Type"];
                        PLine.AccountNo = (string)config["Account_No"];
                        PLine.AccountName = (string)config["Account_Name"];
