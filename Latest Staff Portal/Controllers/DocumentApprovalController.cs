...
namespace Latest_Staff_Portal.Controllers
{
    public class DocumentApprovalController : Controller
    {
                    ...
                    else
                    {
                        ...
                        using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                        {
                            ...
                            foreach (JObject config in details["value"])
                            {
                                ...
                                if ((string)config["Table_ID"] == "70135469" && (string)config["Document_Type"] != "ItemCash")
                                {
                                    DocCount.ImpCount = DocCount.ImpCount + 1;
                                }
                                if ((string)config["Table_ID"] == "70135469" && (string)config["Document_Type"] == "ItemCash")
                                {
                                    DocCount.ItemCashCount = DocCount.ItemCashCount + 1;
                                }
                                if ((string)config["Table_ID"] == "70135450" && (string)config["Document_Type"] != "ItemCash")
                                {
                                    DocCount.SurrCount = DocCount.SurrCount + 1;
                                }
                                if ((string)config["Table_ID"] == "70135450" && (string)config["Document_Type"] == "ItemCash")
                                {
                                    DocCount.ItemCashSurr = DocCount.ItemCashSurr + 1;
                                }
            ...
            try
            {
                ...
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    ...
                    foreach (JObject config in details["value"])
                    {
                        ...
                        PurchaseDoc.Dim3 = CommonClass.GetDimensionValue((string)config["Department_Name"]);
                        ...
                        PurchaseDoc.Proc_Method = (string)config["Procurement_Method"];
                        PurchaseDoc.Proc_Officer = (string)config["Responsible_Officer"];
        ...
        [HttpPost]
        public JsonResult ApproveUnits(string DocNo, List<StudentReqLines> ReqUnits)
        {
            try
            {
                foreach (var c in ReqUnits)
                {
                    ...
                    bool App = c.Approved;
                    Credentials.ObjNav.ApproveStudentRequisitionLines(DocNo, unit, Convert.ToInt32(Ln), App,false);
                }
        ...
        public JsonResult AssignDriverVehicle(string DocNo, string Driver, string Vehicle)
        {
            try
            {
                ...
                if (Session["Username"] != null)
                {
                    ...
                    Credentials.ObjNav.AssignTransportRequisitionDriver(DocNo, Driver, Vehicle, userID,false,false,"","",0);
                    rtVal = true;
