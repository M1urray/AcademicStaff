    ...
    public class DocumentApprovalController : Controller
    {
            ...
            try
            {
                ...
                if (TbID != "")
                {
                    ...
                    if (!string.IsNullOrEmpty(searchValue))
                    {
                        if (Status == "Rejected")
                        {
                            page = "StudentReqApprovalList?$top=" + length + "&$skip=" + start + "&$select=Entry_No,Table_ID,Document_No,Sender_ID,Date_Time_Sent_for_Approval,Status,Sequence_No,Sender_Code,SenderNames,Commented_On,ReqType&$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "'" +
                                " and Status eq '" + Status + "' and contains('" + searchValue.ToLower() + "',tolower(Sender_Code))&$format=json";
                        }
                        else
                        {
                            if (Type == "2")
                            {
                                page = "StudentReqApprovalList?$top=" + length + "&$skip=" + start + "&$select=Entry_No,Table_ID,Document_No,Sender_ID,Date_Time_Sent_for_Approval,Status,Sequence_No,Sender_Code,SenderNames,Commented_On,ReqType&$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "'" +
                                    " and Status eq '" + Status + "' and Commented_On eq true and contains('" + searchValue.ToLower() + "',tolower(Sender_Code))&$format=json";
                            }
                            else
                            {
                                page = "StudentReqApprovalList?$top=" + length + "&$skip=" + start + "&$select=Entry_No,Table_ID,Document_No,Sender_ID,Date_Time_Sent_for_Approval,Status,Sequence_No,Sender_Code,SenderNames,Commented_On,ReqType&$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "'" +
                                    " and Status eq '" + Status + "' and Commented_On eq false and contains('" + searchValue.ToLower() + "',tolower(Sender_Code))&$format=json";
                            }
                        }
                    }
                    else
                    {
                        if (Status == "Rejected")
                        {
                            page = "StudentReqApprovalList?$top=" + length + "&$skip=" + start + "&$select=Entry_No,Table_ID,Document_No,Sender_ID,Date_Time_Sent_for_Approval,Status,Sequence_No,Sender_Code,SenderNames,Commented_On,ReqType&$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "' and Status eq '" + Status
                                     + "'&$format=json";
                        }
                        else
                        {
                            ...
                            {
                                page = "StudentReqApprovalList?$top=" + length + "&$skip=" + start + "&$select=Entry_No,Table_ID,Document_No,Sender_ID,Date_Time_Sent_for_Approval,Status,Sequence_No,Sender_Code,SenderNames,Commented_On,ReqType&$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "' and Status eq '" + Status
                                    + "' and Commented_On eq true&$format=json";
                            }
                            else
                            {
                                page = "StudentReqApprovalList?$top=" + length + "&$skip=" + start + "&$select=Entry_No,Table_ID,Document_No,Sender_ID,Date_Time_Sent_for_Approval,Status,Sequence_No,Sender_Code,SenderNames,Commented_On,ReqType&$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "' and Status eq '" + Status
                                    + "' and Commented_On eq false&$format=json";
                            }
                        }
                    }
                    HttpWebResponse httpResponse = Credentials.GetOdataData(page);
                ...
                if (!string.IsNullOrEmpty(searchValue))
                {
                    if (Type == "2")
                    {
                        page = "StudentReqApprovalList?$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "' and Status eq '" + Status + "'" +
                            " and Commented_On eq true and contains('" + searchValue.ToLower() + "',tolower(Sender_Code))&$format=json";
                    }
                    else
                    {
                        page = "StudentReqApprovalList?$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "' and Status eq '" + Status + "'" +
                            " and Commented_On eq false and contains('" + searchValue.ToLower() + "',tolower(Sender_Code))&$format=json";
                    }
                }
                else
                {
                    if (Type == "2")
                    {
                        page = "StudentReqApprovalList?$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "' and Status eq '" +
                            Status + "' and Commented_On eq true&$format=json";
                    }
                    else
                    {
                        page = "StudentReqApprovalList?$filter=Table_ID eq " + Convert.ToInt32(TbID) + " and Approver_ID eq '" + userID + "' and Status eq '" +
                            Status + "' and Commented_On eq false&$format=json";
                    }
                }
                HttpWebResponse httpResponse = Credentials.GetOdataData(page);
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
        ...
        public PartialViewResult StudentRequisitionDocDetails(string DocNo, string Sequence)
        {
            try
            {
                ...
                string page = "StudentRequisitions?$filter=Code eq '" + DocNo + "'&$format=json";
                HttpWebResponse httpResponse = Credentials.GetOdataData(page);
                using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                {
                    var result = streamReader.ReadToEnd();
                    ...
                    foreach (JObject config in details["value"])
                    {
                        ...
                        RDoc.StudentNo = (string)config["Student_No"];
                        RDoc.StudentName = (string)config["Names"];
                        RDoc.RegType = (string)config["Requisition_Type"];
                        RDoc.DateApplied = Convert.ToDateTime((string)config["Date"]).ToString("dd/MM/yyyy");
                        RDoc.Status = (string)config["Status"];
                        RDoc.Programme = (string)config["Current_Programme"] + "(" + (string)config["Programme_Name"] + ")";
        ...
        [HttpPost]
        public JsonResult ApproveDocument(string DocNo, string EntryNo, string TBID)
        {
            try
            {
                string userID = Session["UserID"].ToString();
