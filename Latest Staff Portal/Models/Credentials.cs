...
namespace Latest_Staff_Portal.Models
{
    public class Credentials
    {
        ...
        public static string fileDestinationPath = @"C:\Portals\Published\Staff\Downloads\";
        ...
        private static DirectorySearcher GetDirectorySearch(string username, string password, string domain)
        {
            if (dirSearch == null)
            {
                try
                {
                    dirSearch = new DirectorySearcher(new DirectoryEntry("LDAP://dc=jooust,dc=ac,dc=ke", username, password));
                }
        ...
        public static string ResetPassword(string username, string newpass)
        {
            ...
            try
            {
                ...
                using (PrincipalContext pContext = new PrincipalContext(ContextType.Domain, "192.168.1.2", AdminAccountName, AdminPassword))
                {
